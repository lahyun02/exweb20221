...
//	: 1개의 요청을 처리하는 동안에만 유지. (요청마다 새로운 객체 생성/ 요청처리 후 객체 소멸 )
//- HttpSession (세션객체) 
//	: 클라이언트(웹브라우저)마다 1개씩 객체를 생성하여 서버가 실행중인 동안 유지. 사용자별로 액션을 나눠서 유지하고 싶을때.
//한번 로그인하면 로그인한 정보를 세션에 저장. -> 그렇지않으면, 요청객체에 요청을 보낼때마다 다시 로그인해야함.
//	-> 사용자별로 독립적으로 유지해야하는 데이터를 저장
// 	  일정시간동안 요청이 없는 세션객체는 삭제 가능 
//- ServletContext (서블릿컨텍스트) 
//  : 웹 애플리케이션 전체에 1개만 존재(서버 종료시까지 유지). 
//	-> 모든 사용자가 공유하는 데이터를 저장

@WebServlet("/attr.do")  //괄호 안에 주소는 각자 정하는 부분.
public class AttrServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (reqCnt == null) {	// 저장되어 있는 값이 없는 경우 
			reqCnt= new AtomicInteger(0);	// 정수 0을 저장한 정수객체 생성 
		}
		...
		if (ssCnt == null) {	// 저장되어 있는 값이 없는 경우 
			ssCnt= new AtomicInteger(0);	// 정수 0을 저장한 정수객체 생성 
		}
		ssCnt.incrementAndGet(); // 정수객체의 값을 1증가
		//세션객체에 "cnt"라는 이름으로 정수값을 저장 
		session.setAttribute("cnt", ssCnt); //1 증가시킨 걸 다시 cnt라는 이름으로 저장	
		
		
		ServletContext context = req.getServletContext();	//현재 앱의 서블릿컨텍스트객체 
		...
		if (scCnt == null) {	// 저장되어 있는 값이 없는 경우 
			scCnt = new AtomicInteger(0);	// 정수 0을 저장한 정수객체 생성 
		}
		scCnt.incrementAndGet(); // 정수객체의 값을 1증가
		//서블릿컨텍스트객체에 "cnt"라는 이름으로 정수값을 저장 
		context.setAttribute("cnt", scCnt); //1 증가시킨 걸 다시 cnt라는 이름으로 저장	
